...
{   
    ...
    //三角形的三个顶点
    Eigen::Vector2f pointA(_v[0].x(), _v[0].y());
    Eigen::Vector2f pointB(_v[1].x(), _v[1].y());
    Eigen::Vector2f pointC(_v[2].x(), _v[2].y());
    
    //三角形三条边
    Eigen::Vector2f edgeAB = pointB - pointA;
    Eigen::Vector2f edgeBC = pointC - pointB;
    Eigen::Vector2f edgeCA = pointA - pointC;

    //像素中心点到三角形三个顶点的向量
    Eigen::Vector2f Ap = p - pointA;
    Eigen::Vector2f Bp = p - pointB;
    Eigen::Vector2f Cp = p - pointC;

	//计算叉积
    float z0 = edgeAB.x() * Ap.y() - edgeAB.y() * Ap.x();
    float z1 = edgeBC.x() * Bp.y() - edgeBC.y() * Bp.x();
    float z2 = edgeCA.x() * Cp.y() - edgeCA.y() * Cp.x();
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for (int x = static_cast<int>(std::floor(minX)); x <= static_cast<int>(std::ceil(maxX)); ++x)
    {
            ...
            
        {
            ...
            {
                //计算插值后的深度
                std::tuple<float, float, float> bary = computeBarycentric2D(x, y, t.v);
